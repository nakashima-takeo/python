...
class Painter():
    def __init__(
        ...
        self.canvas_data = [
            [self.init_color for y in range(self.canvas_width)]
    ...
    def load(self):
        ...
        #print(json_data)
        width, height = json_data['size']
        if width==self.canvas_width and height==self.canvas_height:
            print("Compatible file") 
            for pixel in json_data['pixels']:
                self.canvas_data[pixel[0]][pixel[1]] = pixel[2]
            self.init_canvas()
            self.ui.refresh()
    ...
    def reset(self):
        ...
        for x in range(self.canvas_height):
            for y in range(self.canvas_width):
              self.canvas_data[x][y] = self.init_color
        self.init_canvas()
        self.ui.refresh()
...
class ControlPanel(tk.Frame):
    ...
    def create_ui(self):
        ...
        tk.Button(
            self, text = "reset", command = self.painter.reset
            ).grid(
                row=6, column=0,
                rowspan = 1, columnspan=2
                )    
